...
class DatasetteDepthCapture:
    ...
    # Show display with depth
    def show_depth_map_segmentation(self):
        if self.depth_frame is not None:
            dframe = self.depth_frame.copy()
            # dframe[dframe > 900] = 2**16 - 1
            # dframe[dframe < 700] = 2**16 - 1
    ...
    # Show instructions
    def show_instructions(self, frame, orig, font=cv2.FONT_HERSHEY_SIMPLEX, size=1, color=(240,180,100), thickness=2):
        inst = "q: quit | r: start ROI | s: save ROI"
        cv2.putText(
            frame, 
            f"{inst}", 
            orig, 
    ...
    def capture_hand(self):
        ...
        with dai.Device(self.pipeline) as device:
            ...
            while True:
                ...
                self.show_instructions(datasette_frame, orig=(50,100),color=(240,180,100))
                # Show roi
...
if __name__ == "__main__":
    ...
    if args.mode == 0:
        ...
        ts = datetime.now().strftime("%d/%m/%Y %H:%M:%S") 
        if datasette.depth_roi is not None:
            # Save to file
